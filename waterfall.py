...
def move_water(grid, x, y):
    ...
    if is_move_ok(grid, x, y + 1):  # og conditions
        move_down(grid, x, y)  # run the move down command
        return grid
    if is_move_ok(grid, x - 1, y + 1):  # change to if down is blocked
        down_left(grid, x, y)  # run the down left command
        return grid
    if is_move_ok(grid, x + 1, y + 1):  # change to if left is blocked
        down_right(grid, x, y)
        return grid
    grid.set(x, y, None)
    return grid
...
def move_down(grid, x, y):
    if is_move_ok(grid, x, y+1):
        droplet = grid.get(x, y)
        grid.set(x, y + 1, 'w')  # makes the drop move down
        grid.set(x, y, None)   # sets the square in which the droplet was to None
    return False
...
def down_left(grid, x, y):
    if is_move_ok(grid, x - 1, y + 1):
        droplet = grid.get(x, y)
        grid.set(x - 1, y + 1, 'w')  # makes the drop move down and left one
        grid.set(x, y, None)  # sets the square in which the droplet was to None
...
def down_right(grid, x, y):
    if is_move_ok(grid, x + 1, y + 1):
        droplet = grid.get(x, y)
        grid.set(x + 1, y + 1, 'w')  # makes the drop move down and right one
        grid.set(x, y, None)
