...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    Assume there is sand at x_from,y_from. Is it possible
    to move it to x_to,y_to?
    The from location has sand and is in bounds, but everything
    else must be checked.
    Return True if the move is ok, or False otherwise.
    Ok move: to location is in bounds, empty, not violating corner rule.
    ...
    False
    >>>>>> is_move_ok(grid, 0, 0, 10, 10) # right-up OOB  # my doctest
    False
    >>> is_move_ok(grid, 0, 0, -1, 10) # up left OOB  # my doctest
    False
    >>> # pass - implement 2 more OOB tests here/ done
    ...
    """
    if grid.in_bounds(x_from, y_from):  # if the initial x,y is in bounds
        if grid.in_bounds(x_to, y_to):  # if the target x,y is in bounds
            if grid.get(x_to, y_to) is None:
                if grid.get(x_to, y_to):
                    return True
                if grid.get(x_to, y_from):
                    return True
...
def do_gravity(grid, x, y):
    ...
    if grid.get(x, y) == 's':
        if is_move_ok(grid, x, y, x, y + 1):    # checks if the move to the bottom square is okie dokie
            do_move(grid, x, y, x, y + 1)
            return grid  # return the original grid
        if is_move_ok(grid, x, y, x - 1, y + 1):  # checks if the move to the bottom-left square is okay
            do_move(grid, x, y, x - 1, y + 1)
            return grid
        if is_move_ok(grid, x, y, x + 1, y + 1):  # checks if the move to the bottom-right square is okay
            do_move(grid, x, y, x + 1, y + 1)
            return grid
